...
const NewArrival = () => {
    ...
    const { isLoading, isError, message, products } = useAppSelector((state) => state.product)
    useEffect(() => {
    ...
    return (
        <Box>
            {!isLoading && <HStack justify='between'>
                <Textfield title="New Arrival" fontSize={18} fontFamily='Poppins_700Bold' color={colors.text_dark} />
            ...
            <Box mt={10}>
                {isLoading ? <Text>Loading</Text> : (
                    <Box style={{
                        ...
                        {products?.products.map((item: ProductArray, index: number) => (
